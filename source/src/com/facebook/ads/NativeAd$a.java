    ...
    implements android.view.kListener, android.view.hListener
{
    ...
    {
        boolean flag = false;
        if (motionevent.getAction() == 0 && NativeAd.h(a) != null)
        {
            d = NativeAd.h(a).getWidth();
            e = NativeAd.h(a).getHeight();
            int ai[] = new int[2];
            NativeAd.h(a).getLocationInWindow(ai);
            f = ai[0];
            g = ai[1];
            Rect rect = new Rect();
            NativeAd.h(a).getGlobalVisibleRect(rect);
            i = rect.width();
            h = rect.height();
            int ai1[] = new int[2];
