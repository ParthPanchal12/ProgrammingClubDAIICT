...
_L7:
    ...
    {
        if (view != null && view.getParent() != null && view.getVisibility() == 0 && (android.os.Build.VERSION.SDK_INT < 11 || view.getAlpha() >= 0.9F))
        {
            int k = view.getWidth();
            ...
            if (ai[0] >= 0 && ((DisplayMetrics) (view)).widthPixels - ai[0] >= k)
            {
                ...
                if ((ai[1] >= 0 || Math.abs(ai[1]) <= i) && (j + ai[1]) - ((DisplayMetrics) (view)).heightPixels <= i)
                {
                    return a(context);
                }
            }
